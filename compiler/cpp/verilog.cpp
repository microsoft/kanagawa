...
class VerilogCompiler
{
        ...
        for (size_t i = 0; i < _program._registerTable.size(); ++i)
        {
            ...
            if (regDesc._type == RegisterType::Global)
            {
                ...
                if (nameToContainer.end() == containerIt)
                {
                    ...
                    const mlir::Value clk = circt::seq::ToClockOp::create(opb, location, untypedClock);
                    ...
                    {
                        valueReg = circt::seq::CompRegOp::create(opb, location, valueBackedge, clk, symbol);
                    }
                    ...
                    for (size_t j = 0; j < key._writeCount; ++j)
                    {
                        ...
                        valueNext =
                            circt::comb::MuxOp::create(opb, location, inputValid, inputData, valueNext, TwoState);
                    }
                    ...
                    if (key._tracksRaces)
                    {
                        ...
                        const mlir::Value sum =
                            PopCount(opb, location, writeEnables,
                                     circt::hw::ConstantOp::create(opb, location, opb.getIntegerAttr(GetI1Type(), 0)));
    ...
    mlir::Value InstantiateMemoriesCIRCT()
    {
        ...
        for (size_t i = 0; i < _program._registerTable.size(); ++i)
        {
            ...
            if (regDesc._type == RegisterType::Memory)
            {
                ...
                mlir::Value memInitDone =
                    circt::hw::ConstantOp::create(opb, location, opb.getIntegerAttr(opb.getI1Type(), 1));
                if (hasInitialData)
                {
                ...
                for (size_t moduleIndex = 0; moduleIndex < numModules; ++moduleIndex)
                {
                    ...
                    if (useLogicRam)
                    {
                        ...
                        lramDataModuleReadAddr =
                            circt::hw::ArrayCreateOp::create(opb, location, lramDataModuleReadAddrArray);
                    ...
                    else if (isQuadPort)
                    {
                        ...
                        memoryDataModuleWriteAddr =
                            circt::hw::ArrayCreateOp::create(opb, location, memoryDataModuleWriteAddrArray);
                        memoryDataModuleDualPortWren =
                            circt::hw::ArrayCreateOp::create(opb, location, memoryDataModuleDualPortWrenArray);
                        memoryDataModuleDualPortDataIn =
                            circt::hw::ArrayCreateOp::create(opb, location, memoryDataModuleDualPortDataInArray);
                        memoryDataModuleDualPortRden =
                            circt::hw::ArrayCreateOp::create(opb, location, memoryDataModuleDualPortRdenArray);
                        memoryDataModuleReadAddr =
                            circt::hw::ArrayCreateOp::create(opb, location, memoryDataModuleReadAddrArray);
                    }
                    else
                    {
                        llvm::SmallVector<mlir::Value> memoryDataModuleDualPortRdenArray(2);
                        ...
                        if (writePorts > 0)
                        {
                            ...
                            if (writeArbitration)
                            {
                                ...
                                const mlir::Value memoryDataWriteData =
                                    circt::hw::ArrayCreateOp::create(opb, location, memoryDataWriteDataPorts);
                                const mlir::Value memoryDataWriteAddr =
                                    circt::hw::ArrayCreateOp::create(opb, location, memoryDataWriteAddrPorts);
                                std::reverse(memoryDataWriteDataPorts.begin(), memoryDataWriteDataPorts.end());
                                std::reverse(memoryDataWriteAddrPorts.begin(), memoryDataWriteAddrPorts.end());
                            ...
                            const mlir::Value reduceWren = circt::comb::OrOp::create(opb, location, wrenArray, TwoState);
                        ...
                        memoryDataModuleDualPortAddr =
                            circt::hw::ArrayCreateOp::create(opb, location, memoryDataModuleDualPortAddrArray);
                        memoryDataModuleDualPortDataIn =
                            circt::hw::ArrayCreateOp::create(opb, location, memoryDataModuleDualPortDataInArray);
                        memoryDataModuleDualPortWren =
                            circt::hw::ArrayCreateOp::create(opb, location, memoryDataModuleDualPortWrenArray);
        ...
        if (memInitDoneTerms.empty())
        {
            allMemoryInitialized =
                circt::hw::ConstantOp::create(opb, GetUnknownLocation(), opb.getIntegerAttr(GetI1Type(), 1));
        }
        else
        {
            allMemoryInitialized = circt::comb::AndOp::create(opb, GetUnknownLocation(), memInitDoneTerms, TwoState);
        }
  ...
  private:
...

            {
                ...
                for (const auto& p : offsetToWidth)
                {
                    ...
                    sparseConcat.Insert(sliceOffset,
                                        circt::sv::ReadInOutOp::create(opb, mlirBbLocation, sliceType, inputSliceWire));
                }
                ...
                for (const auto& p : offsetToWidth)
                {
                    ...
                    circt::sv::AssignOp::create(opb, mlirBbLocation, outputSliceWire, slice);
                }
            }
        }
    }
    ...
    StartNewThreadResult CompileStartNewThreadCirct(circt::OpBuilder& opb)
    {
        ...
        result._startConditionWire = circt::sv::WireOp::create(opb, bbLocation, opb.getI1Type(), "start_condition");

        mlir::Value startConditionValue =
            circt::sv::ReadInOutOp::create(opb, bbLocation, opb.getI1Type(), result._startConditionWire);
...

        {
...

            {
                ...
                allowNewThreadPreStartCondition =
                    circt::comb::AndOp::create(opb, bbLocation, allowThreadTerms, TwoState);
            }
                                         ...
                                         const SourceOpToStringMode srcOpToStrMode = SourceOpToStringMode::Default)
    {
        ...
        case SourceOperandType::Fifo:
        {
            ...
            result = circt::comb::ExtractOp::create(opb, location, inputValue, fs._offset, fs._width);
        }
        break;
    ...
    void CompileDequeueRegistersCirct(const Operation& op)
    {
        ...
        for (size_t i = 0; i < op._dst.size(); i++)
        {
            ...
            const mlir::Value slice =
                circt::sv::ReadInOutOp::create(opb, opLocation, GetIntegerType(wireWidth), srcWire);
    ...
    void CompileLoadMemoryCirct(const Operation& op)
    {
        ...
        for (size_t i = 0; i < numDataElements; ++i)
        {
            ...
            if (RegisterType::BitBucket != _program._registerTable[destRegisterIndex]._type)
            {
                circt::comb::ExtractOp extractOp =
                    circt::comb::ExtractOp::create(opb, opLocation, readData, offset, width);
        ...
        if (ecc)
        {
            ...
            for (size_t i = 0; i < 2; i++)
            {
                ...
                {
                    circt::comb::ExtractOp extractOp = circt::comb::ExtractOp::create(opb, opLocation, eccBits, i, 1);
    ...
    void CompileStoreMemoryCirct(const Operation& op)
    {
        ...
        circt::comb::ConcatOp combinedWriteData = circt::comb::ConcatOp::create(opb, opLocation, writeDataFields);
    ...
    void CompileCycleCounterCirct(const Operation& op)
    {
        ...
        const mlir::Value cycleCount =
            circt::sv::ReadInOutOp::create(opb, opLocation, _compileContext._cycleCountReg.getType(), cycleCountWire);
    ...
    void CompileStallCheckCirct(const Operation& op)
    {
        ...
        for (size_t i = 0; i < 8; i++)
        {
            ...
            {
                iVal =
                    circt::hw::ConstantOp::create(opb, opLocation, opb.getIntegerAttr(opb.getIntegerType(LfsrWidth), 0));
            }
            thresholdValues.push_back(iVal);
        }

        const mlir::Value selectIndex =
            circt::sv::ReadInOutOp::create(opb, opLocation, GetIntegerType(3), _compileContext._thresholdSelWire);
        ...
        instance.AddPort("en", circt::hw::ModulePort::Direction::Input,
                         circt::hw::ConstantOp::create(opb, opLocation, opb.getIntegerAttr(opb.getI1Type(), 1)));
        instance.AddPort("lfsr_out", circt::hw::ModulePort::Direction::Output,
                         GetIntegerType(c_stallLfsrBits));
        instance.Generate(&opb);
    ...
    void CompileBypassMemoryCirct(const Operation& op)
    {
        ...
        for (size_t i = 0; i < numOperands; ++i)
        {
            ...
            const mlir::Value resultSubset =
                circt::comb::ExtractOp::create(opb, opLocation, instance.GetPortValue("read_data_out"), offset, width);
            ...
            if (op._dst.size() > 0)
            {
                ...
                for (size_t i = 0; i < op._dst.size(); i++)
                {
                    ...
                    const mlir::Value resultSlice =
                        circt::comb::ExtractOp::create(opb, opLocation, resultVal, offset, width);
                    offset += width;
                                                 ...
                                                 const std::function<mlir::Value(void)>& callback)
    {
        ...
        if (generateStageIndex != currStageIndex)
        {
...

            {
                ...
                circt::pipeline::LatencyReturnOp::create(opb, location, resultValues);
            }
    ...
    void CloseCirctBasicBlock()
    {
        ...
        for (const BasicBlockPortInfo& port : _compileContext._basicBlockPorts)
        {
            if (circt::hw::ModulePort::Direction::Output == port._portInfo.dir)
            {
                ...
                const mlir::Value value = circt::sv::ReadInOutOp::create(opb, mlirBbLocation, port._portInfo.type, wire);
        ...
        circt::hw::OutputOp::create(opb, LocationToCirctLocation(basicBlock._location), outputs);
            ...
            if (circt::hw::ModulePort::Direction::Input == port.dir)
            {
                ...
                else
                {
                    ...
                    circt::sv::VerbatimExprOp verbatimOp =
                        circt::sv::VerbatimExprOp::create(opb, _location, verbatimType, value);
